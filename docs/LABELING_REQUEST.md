...
- **누가.** **라벨러A과 공동 연구자** 두 사람이 같은 포스터를 **따로** 긋는다. 연습 포스터 2장만 같이 본다. 본 라벨링에서는 서로의 결과를 보지 않는다.
- **무엇으로.** 가이드 긋기 도구 `선긋기.html` — 파일 하나다. ~~**크롬(Chrome)으로** 연다~~ (8절). [사후 정정 1] — 실제로는 두 라벨러 모두 Safari 로 작업했다.
...
> **실제 작업 환경: Safari.** 두 라벨러 모두 Safari 로 작업했다. 아래 «크롬으로 연다» 는 요청서를
> 보낼 때(2026-09-14) 적은 **지시**이고, 실제와 다르다. [사후 정정 1] 을 본다.

**열기.** ~~`선긋기.html` 을 **크롬(Chrome)으로** 연다 — 더블클릭해서 다른 브라우저가 열리면 크롬 창에 파일을 끌어다 놓는다.~~ [사후 정정 1] 인터넷이 없어도 된다. 홈에 할 일 세 단계와 참조 그림이 있다. 이름을 적고 «연습 포스터» 또는 «본 라벨링» 을 누른다.
...
- **자동 저장.** 긋는 대로 브라우저에 저장된다 (위쪽 «자동 저장됨 HH:MM»). 같은 컴퓨터 · 같은 브라우저에서 도구를 다시 열면 홈에 «계속하기» 가 뜬다. «초기화» 는 브라우저에 남은 작업을 지운다. **크롬을 권장한다.** [사후 정정 1 — 실제 작업 환경은 Safari] 다른 브라우저(Safari · Firefox 등)에서 파일을 직접 열면 자동 저장이 안 될 수 있다 — 어느 브라우저든 **중간중간 내보내기** 한다 (한두 시간마다, 그리고 하루가 끝날 때).
...
3. **라벨러A에 관한 기록.** 두 라벨러의 차이를 해석할 때 아래 사실을 함께 적는다.
   - 라벨러A은 이 포스터들의 검출 결과(Surya 줄 · 블록)와 1단계 결과를 보지 않았다 (2026-09-16 정정 — 전에는 «이미 보았다» 로 잘못 적었다).
...
7. **이 자료의 지위.** 정답이 아니라 참조다. 라벨러는 두 명(라벨러A · 공동 연구자)이고, 만든 이 · 날짜 · 도구가 파일 안에 적힌다. 받으면 `docs/HUMAN_DATA.md` D절의 행을 B절로 옮긴다.
...
*(요청서를 보낸 뒤에 알게 된 사실을 적는다. 원래 문구는 지우지 않고 취소선으로 남긴다. 라벨링이
끝난 뒤 더한 것이므로 «지시» 가 아니라 «기록» 이다.)*

**[사후 정정 1] 브라우저 — 지시는 크롬, 실제 작업 환경은 Safari (적은 날 2026-09-21)**

- **무엇이 달랐나.** 0절 · 8절은 `선긋기.html` 을 크롬으로 열라고 적었고, 9절은 «크롬을 권장한다 ·
  다른 브라우저(Safari · Firefox 등)에서 파일을 직접 열면 자동 저장이 안 될 수 있다» 고 적었다.
  **두 라벨러 모두 Safari 로 작업했다.** 세 자리 모두 이 정정이 적용된다.
- **어떻게 아는가.** 연구자가 알려 준 사실이다. 도구(`guides-tool 2.0`)와 내보낸 라벨 파일은
  브라우저 · 운영체제를 **기록하지 않는다** — `tool` 에는 버전과 판본 해시만, 포스터마다는 작업 시간 ·
  배율 · 고친 횟수만 남는다. 그래서 파일에서 확인할 수 있는 근거는 없다.
- **자료에 미치는 영향으로 확인된 것.** 두 벌 모두 정상적으로 내보내졌고(`guides_labelerA_20260917-2016.json` ·
  `guides_labelerB_20260916-0201.json`), `eval/brockmann_stage2_score.py check` 의 수령 검증을 통과했다.
  9절이 적은 자동 저장 위험이 실제로 작업 손실을 냈는지는 파일로 확인할 수 없다.
- **고치지 않은 것.** 요청서의 원래 문구, 이미 보낸 PDF(`docs/labeling/가이드긋기_요청서.pdf`), 라벨 파일.
