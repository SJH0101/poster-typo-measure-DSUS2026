...
- **라벨러 노출.**
  - 라벨러A은 이 50장의 검출 결과와 1단계 결과를 보지 않았다 (2026-09-16 정정 — 전에는 «검출 결과와 측정값을 봤다» 로 잘못 적었다). 32 · 40 · 49 번에는 탐색 단계에서 다른 도구로 상자를 그은 적이 있다.
...
- 넣은 두 장은 IDML 가이드 · 라벨러A 상자와 겹치지 않는다.
