...
def merge_c_in(t):
    """c_in 쌍으로 이어진 블록을 하나로 합친 참값 (docs/constants_preregister.json 수정 2 의 5 · 수정 12 의 2).

    c_in 은 경계 간격이 블록 안 줄 사이와 같은 자리다 — 사전등록 docs/clean_preregister.json «조작변인 — 여유 c» ›
    «근거»: «c_in 은 경계 간격이 블록 안과 같아 C 가 가를 정보가 없고 사람 라벨 규칙으로도 병합인 자리.»
    블록 채점의 참조만 바꾼다. 쌍 병합률 곡선은 원 정의 (합치지 않은 참값) 그대로 쓴다.
    """
    idx = {tb['id']: i for i, tb in enumerate(t['blocks'])}
    par = list(range(len(t['blocks'])))
    ...
    for q in t['pairs']:
        if str(q.get('level')) == 'c_in':
            a_, b_ = find(idx[q['upper']]), find(idx[q['lower']])
            if a_ != b_:
    ...
    for root in sorted(groups):
        ids = groups[root]
        bs = [t['blocks'][i] for i in ids]
        ls = sorted((ln for b in bs for ln in b['lines']), key=lambda ln: ln['baseline_y'])
        box = [min(b['ink_box'][0] for b in bs), min(b['ink_box'][1] for b in bs),
               max(b['ink_box'][2] for b in bs), max(b['ink_box'][3] for b in bs)]
        nb = dict(bs[0]); nb.update(id='+'.join(b['id'] for b in bs), n=sum(b['n'] for b in bs), lines=ls, ink_box=box,
                                    c_in_합침=len(bs) > 1)
        blocks.append(nb)
    out = dict(t); out['blocks'] = blocks
    out['pairs'] = []      # 쌍은 원 정의로만 센다 (합친 블록은 id 가 달라 쌍 조회가 안 된다)
    return out
...
def score_clean(a):
    ...
    per = {m: clean_method(items, L, truths, asg[m], src_of=({k: Cdiag[k]['source'] for k in C} if m == 'C' else None))
           for m in order}
    # 블록 채점의 참조 — c_in 쌍을 한 블록으로 합친 것이 본 수치, 합치지 않은 것이 참고 열 (수정 2 의 5 · 수정 12 의 2)
    truths_m = {k: merge_c_in(t) for k, t in truths.items()}
    asg_m = dict(asg)   # 오라클은 그 참조의 블록으로 묶는다 — 합친 참조에서는 합친 블록으로 (수정 2 의 취지)
    asg_m['오라클'] = {k: groups_oracle(L[k]['lines'], truths_m[k]) for k, _p, _it in items}
    per_m = {m: clean_method(items, L, truths_m, asg_m[m], src_of=({k: Cdiag[k]['source'] for k in C} if m == 'C' else None))
             for m in order}
    ...
    res['블록'] = {m: clean_blocks(per_m[m], strata) for m in order}
    res['블록_참고_원정의'] = {m: clean_blocks(per[m], strata) for m in order}
    res['정의']['블록_참조'] = ('본 수치 = c_in 쌍을 한 블록으로 합친 참값 (constants_preregister 수정 2 의 5 · 수정 12 의 2, '
                             'group_score.merge_c_in). 참고 열 «블록_참고_원정의» = 합치지 않은 원 정의. 쌍 병합률 곡선은 원 정의 그대로')
    res['블록_참값_수'] = dict(
        층별={_sname(s): dict(원정의=sum(len(t['blocks']) for k, t in truths.items() if _stratum(t) == s),
                            합친정의=sum(len(t['blocks']) for k, t in truths_m.items() if _stratum(truths[k]) == s),
                            c_in_쌍=sum(sum(1 for q in t['pairs'] if str(q.get('level')) == 'c_in')
                                      for t in truths.values() if _stratum(t) == s)) for s in strata},
        합계=dict(원정의=sum(len(t['blocks']) for t in truths.values()), 합친정의=sum(len(t['blocks']) for t in truths_m.values()),
                c_in_쌍=sum(sum(1 for q in t['pairs'] if str(q.get('level')) == 'c_in') for t in truths.values())))
