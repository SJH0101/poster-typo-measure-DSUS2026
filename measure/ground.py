...
def measure_boxes(path, boxes, coords='norm'):
    ...
    for i, b in enumerate(boxes):
        ...
        # 가로로 나란한 두 블록이 한 상자로 들어오면 갈라서 따로 잰다 (region.split_columns, 2026-09-18).
        # 빈틈 기준은 그 블록에서 잰 x높이 — 새 상수는 없다. 갈린 조각은 id 에 -1 · -2 를 붙인다.
        parts = region.split_columns(g, px, m.get('xh_median')) if m.get('n_lines') else [px]
        if len(parts) > 1:
            got = [(q, region.measure(g, q)) for q in parts]
            if all(x[1].get('n_lines') for x in got):
                for j, (q, mm) in enumerate(got, 1):
                    out.append(dict(id=f'{bid}-{j}', box_px=[round(v, 1) for v in q],
                                    role=(b.get('role') if isinstance(b, dict) else None), **mm))
                continue
