...
# 발음기호 흡수 · 어센더 판정 — docs/constants_preregister.json 수정 6 (순서 2 정의 유도, Helvetica 1000px 실측).
# 유도값은 x높이 기준 비이고 lines() 는 띠 높이 nb.h (어센더 윗끝 ~ 베이스라인) 에 곱하므로 어센더 ÷ x높이 = 1.3748 로 나눈다.
MARK_SMALL = 0.284      # 뜬 조각 높이 ÷ 띠 높이 상한 = 0.3901 (å 고리 ÷ x높이) ÷ 1.3748
MARK_GAP_RATIO = 0.131  # 물러남 경로에만 쓴다 (수정 7) — 조각 ~ 띠 틈 ÷ 띠 높이 = 0.1797 ÷ 1.3748
MARK_GAP_MIN = 3        # 물러남 경로의 바닥 (행) = 번진 가장자리 1행 + 점 1행 + 몸통 1행
MARK_GAP_RATIO_XH = 0.1797  # 수정 7 — 틈 문턱 = ⌈MARK_GAP_RATIO_XH × nb x높이⌉ + MARK_GAP_E.
MARK_GAP_E = 1              # 0.1797 = i 점 아래끝과 x높이선 거리 ÷ x높이 (Helvetica 실측), + 1 = e (수정 4)
ASC_RATIO = 0.3748      # (어센더 − x높이) ÷ x높이 = (0.719 − 0.523) ÷ 0.523. 어센더 판정 문턱 = max(2, ⌊ASC_RATIO × x높이_px ÷ 2⌋)
...
def lines(g, th, x0, x1, min_h=2, body_h=4, mask=None):
    ...
    # 수정 7 — 흡수 문턱은 이웃 줄의 x높이로 잰다. baseline · split_marks 는 lines() 결과를 쓰지 않아 순환이 없다.
    # 메모 열쇠에 띠 구간이 들어가므로 흡수로 띠가 넓어지면 자연히 다시 잰다. 못 재면 띠 높이 식으로 물러난다.
    _xh_memo = {}

    def nb_xh(o):
        k = (o['s'], o['e'], o['span'])
        if k not in _xh_memo:
            b, _raw = baseline(ink, o['s'], o['e'])
            f = split_marks(m, o['s'], o['e'], o['span'], b)
            _xh_memo[k] = (b - f['x_top']) if f['x_top'] is not None else 0
        return _xh_memo[k]

    def gap_thr(o):
        xh = nb_xh(o)
        if xh and xh > 0:
            return math.ceil(MARK_GAP_RATIO_XH * xh) + MARK_GAP_E
        return max(MARK_GAP_MIN, o['h'] * MARK_GAP_RATIO)
    ...
    while i < len(out):
        ...
        for j in (i + 1, i - 1):
            ...
            small = (cur['h'] <= nb['h'] * MARK_SMALL
                     and (wide(cur) <= FRAG_WIDE * max(wide(nb), 1)
                          or cover(cur) <= FRAG_COVER))
            if inside and small and gap <= gap_thr(nb) and (bd is None or gap < bd):
                best, bd = j, gap
        if best is not None:
            nb = out[best]
...
def split_marks(m, s, e, span, base):
    ...
    cap = min(tops)
    # 몸통 조각 대다수가 x 높이에서 시작하면 어센더 없음. 문턱은 x높이에 비례 (수정 6): x높이_px = base − xh
    kind = 'ascender' if (xh - cap) >= max(2, int(np.floor(ASC_RATIO * (base - xh) / 2))) else 'x_only'
    if kind == 'x_only':
        cap = None
