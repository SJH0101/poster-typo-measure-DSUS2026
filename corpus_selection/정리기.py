...
import csv, os, re, shutil, threading
...
SERIES_RULES = [
    ...
    ("Opernhaus",        ["opernhaus"]),
    ("Schauspielhaus",   ["schauspielhaus", "stadttheater"]),
    ("Tonhalle_Konzert", ["tonhalle", "konzert", "kammermusik"]),
    ("Volg",             ["volg"]),
    ("BEA",              ["bea", "viscount"]),
    ("Verkehr",          ["schützt", "lärm", "bruit", "automobil", "radfahrer",
                          "cycliste", "strassenbau", "achtung", "velo",
                          "überholen", "routier"]),
    ("Ausstellung",      ["kunstgewerbemuseum", "ausstellung", "kunsthaus",
                          "gewerbemuseum", "helmhaus"]),
]
...
def fmt_family(r):
    try:
        ...
        return "불명"
    if h <= 0 or w <= 0 or abs(h / w - WELT) > EPS:
        return "기타판형"
...
class App:
    def __init__(self, root):
        ...
        ttk.Label(g, text="(designer 열이 이 문자열로 시작하면 개인 작업으로 본다. 비우면 전부 포함)",
                  foreground="#666").grid(row=1, column=0, columnspan=2, sticky="w", padx=8)
        ttk.Checkbutton(g, text="복사 대신 이동 (원본이 사라짐)",
                        variable=self.do_move).grid(row=2, column=0, columnspan=2, sticky="w", padx=8, pady=6)
    ...
    def run(self, preview):
        ...
        threading.Thread(target=self._work, args=(preview,), daemon=True).start()
